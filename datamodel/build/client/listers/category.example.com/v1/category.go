...
// List lists all Categories in the indexer.
func (s *categoryLister) List(selector labels.Selector) (ret []*v1.Category, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.Category))
	})
	return ret, err
}

// Get retrieves the Category from the index for a given name.
func (s *categoryLister) Get(name string) (*v1.Category, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("category"), name)
	}
	return obj.(*v1.Category), nil
}
